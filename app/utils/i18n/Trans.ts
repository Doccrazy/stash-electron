...
const Trans = contextualize(
  ({ locale, id, markdown, children, ...context }: Props) => {
  ...
  IdContext,
  (props) => !props.id || props.id.startsWith('.'),
  (props, idFromContext) => ({ ...props, id: `${idFromContext}${props.id || ''}` })
);
