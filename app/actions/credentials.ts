...
export function clearStoredLogin(context: string): Thunk<Promise<void>> {
  return async (dispatch, getState) => {
    ...
    try {
      dispatch(
        Settings.changeAndSave(
          SETTINGS_KEY,
          getState().settings.current.storedLogins.filter((ctx) => ctx !== context)
        ) as any
      );
  ...
  return async (dispatch, getState) => {
    ...
    if (credentials.context && formState.savePassword && formState.password) {
      try {
        ...
        toastr.error(t('component.loginPopup.error.storeCredentials.title'), t('component.loginPopup.error.storeCredentials.message'));
        console.error(e);
      }
    }
    dispatch({
...
type Action =
  | TypedAction<
      Actions.OPEN,
      { context: string; title: string; text: string; username?: string; askUsername?: boolean; defaultSavePassword?: boolean }
    >
