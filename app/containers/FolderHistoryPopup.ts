...
export default connect(
  (state: RootState) => {
    ...
    const commits = state.nodeHistory.open
      ? state.git.history.commits
          .valueSeq()
          .toArray()
          .filter(
            (commit) =>
    ...
    return {
      ...
      title: state.nodeHistory.nodeId
        ? `Commits for ${state.repository.nodes[state.nodeHistory.nodeId].name} (${commits.length})`
        : undefined,
      commits
    };
  },
