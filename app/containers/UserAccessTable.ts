...
function buildAuthNodes(nodes: { [nodeId: string]: Node }) {
  return Set(Object.values(nodes).filter((n) => n.authorizedUsers && n.authorizedUsers.size))
    ...
    .map((node: Node) => ({
      node,
      level: hierarchy(nodes, node).length - 1,
      relevant: !!node.authorizedUsers && !!node.authorizedUsers.size
    }))
    .toArray();
}
...
export default connect(
  ...
  (dispatch: Dispatch, props) => ({
    onToggle: (node: Node, username: string) => {
      dispatch(bulkToggle({ nodeId: node.id, username }));
    }
  })
)(UserAccessTable);
