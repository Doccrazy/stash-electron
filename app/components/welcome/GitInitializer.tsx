...
const GitInitializer = ({
  ...
  <div>
    ...
    {status.initialized && (
      <div>
        <p>
          <i className="fa fa-check text-success" /> A git repository has been set up at the selected location.
        </p>
    ...
    {repoLoaded && !status.initialized && (
      <div>
        <p>
          While it is possible to use Stash purely offline, you will miss out on its collaboration features without using a{' '}
          <b>protected git repository</b>.
        </p>
        <FormGroup>
          <Label check>
            <Input
              type="radio"
              checked={action === GitInitAction.OFFLINE}
              onClick={() => onChangeAction(GitInitAction.OFFLINE)}
              disabled={working}
            />{' '}
        ...
        <FormGroup>
          <Label check>
            <Input
              type="radio"
              checked={action === GitInitAction.INIT}
              onClick={() => onChangeAction(GitInitAction.INIT)}
              disabled={working}
            />{' '}
        ...
        <FormGroup>
          <Label check>
            <Input
              type="radio"
              checked={action === GitInitAction.CLONE}
              onClick={() => onChangeAction(GitInitAction.CLONE)}
              disabled={working}
            />{' '}
            Clone an existing repository from a remote URL
          </Label>
        </FormGroup>
        {action === GitInitAction.CLONE && (
          <FormGroup>
            <InputGroup>
              <Input
                placeholder="Remote repository URL"
                value={cloneUrl || ''}
                onChange={(ev) => onChangeCloneUrl(ev.target.value)}
                disabled={working}
              />
              <InputGroupAddon addonType="append">
                <Button color="info" onClick={onClone} disabled={working}>
                  {working ? <i className="fa fa-spinner fa-pulse" /> : 'Clone'}
                </Button>
              </InputGroupAddon>
            </InputGroup>
