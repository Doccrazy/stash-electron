...
export function dragSource<T>(itemType: string) {
  return DragSource(
    ...
    return connectDragSource!(<span>{children}</span>);
  });
}
...
export function dropTarget<T>(itemType: string) {
  ...
  )(({ children, className, acceptClassName, connectDropTarget, willDrop }: EntryDropTargetProps<T>) =>
    connectDropTarget!(<span className={cx(className, willDrop && acceptClassName)}>{children}</span>)
  );
}
