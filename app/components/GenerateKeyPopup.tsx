...
export default withTrans<Props>('component.generateKeyPopup')(
  ...
  <Modal isOpen={open} toggle={onClose}>
          ...
          <div>
            <ButtonGroup>
              {STRENGTH_OPTIONS.map(str =>
                <Button key={str} outline active={strength === str} onClick={() => onChangeStrength(str)}>{t('.strengthBits', { bits: str})}</Button>
              )}
            </ButtonGroup>
          </div>
        </FormGroup>
    ...
    <ModalFooter>
      {!passphrase && <div style={{ flexGrow: 1 }}><i className="fa fa-warning text-warning" /> {t('.error.emptyPassphrase')}</div>}
      <Button type="submit" form="editForm" color="primary" disabled={!valid || disabled}>
        {disabled && <i className="fa fa-spinner fa-pulse" />}{' '}
        {t('.action.generate')}
      </Button>{' '}
      <Button color="secondary" onClick={onClose} disabled={disabled}>{t('action.common.cancel')}</Button>
    </ModalFooter>
  </Modal>
));
