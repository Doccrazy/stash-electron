...
export default withTrans<Props>()(({ users, authorizationNodes, modifications, currentUser, onToggle, t }) => (
  <table className={`table table-hover table-sm table-sticky ${styles.table}`}>
    <thead>
      <tr>
        <th className="sticky-y">
          <Trans id="common.column.folder" />
        </th>
        {users.map((username) => (
          <th key={username} className={username === currentUser ? 'table-success' : ''}>
            {username}
          </th>
        ))}
      </tr>
          ...
          {users.map((username) => {
            ...
            return (
              <td
                ...
                onClick={() => {
                  if (mayEdit) {
                    onToggle(node, username);
                  }
                }}
                title={mayEdit ? t('action.folder.toggleAccess') : ''}
                className={cx(
                  auth.relevant && (authorized ? styles.authorized : styles.unauthorized),
                  mayEdit ? 'clickable' : styles.readonly,
                  modified && styles.modified,
                  username === currentUser && 'table-success'
                )}
              />
            );
          })}
