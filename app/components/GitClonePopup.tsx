...
export default withTrans<Props>('component.gitClonePopup')(({ t, open, working, valid, feedback, url, target, onChangeUrl, onChangeTarget, onClone, onClose }) => {
  return (<Modal isOpen={open} toggle={() => { if (!working) { onClose(); } }}>
    ...
    <ModalBody>
      <Form>
        <FormGroup>
          ...
          <FocusingInput placeholder={t('.field.remoteUrl.placeholder')} focused valid={valid ? undefined : false} disabled={working}
                 value={url || ''} onChange={ev => onChangeUrl(ev.target.value)} />
        </FormGroup>
        <FormGroup>
          <Label>{t('.field.targetFolder')}</Label>
          <FileInput folder dialogTitle={t('.field.targetFolder.dialogTitle')}
                     placeholder={t('.field.targetFolder.placeholder', {example: path.join(os.homedir(), 'Workspace')})}
                     valid={valid ? undefined : false} disabled={working} value={target || ''} onChange={ev => onChangeTarget(ev.target.value)}/>
    ...
    <ModalFooter>
      ...
      <Button color="secondary" onClick={onClose} disabled={working}>{t('action.common.close')}</Button>
    </ModalFooter>
  </Modal>);
});
