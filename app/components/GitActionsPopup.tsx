    ...
    >
      ...
      <ModalBody>
        {!status.conflict && !status.error && status.upstreamName && (
          <p>{formatStatusLine(status.commitsAheadOrigin, status.upstreamName)}</p>
        )}
        {status.incomingCommits && (
          <p className="text-success">{t('common.git.newCommits', { incomingCommits: status.incomingCommits })}</p>
        )}
        ...
        {status.conflict && (
          <div>
            ...
            {status.filesInConflict?.length ? (
              <div className="mb-3">
                {t('.conflict.fileList')}
                <ul>{status.filesInConflict?.map((path) => <li key={path}>{path}</li>)}</ul>
              </div>
            ) : null}
          </div>
        )}
            ...
            <GitCommitsTable
              ...
              rowAction={(commit) =>
                !commit.pushed && (
                  <a
                    href=""
                    onClick={() => revertClick(commit.hash)}
                    title={t('.action.selectRevert')}
      ...
      <ModalFooter>
        ...
        {(!!toPush || !!toReset) && (
          <Button innerRef={doFocus} color={toPush ? 'success' : 'danger'} onClick={onPushRevert} disabled={disabled}>
        ...
        {status.conflict && (
          <Button color="warning" onClick={onResolve} disabled={disabled}>
            {t('action.git.resolveTheirs')}
          </Button>
        )}
        <Button color="info" onClick={onRefresh} disabled={disabled}>
          <i className="fa fa-refresh" /> {t('action.git.pull')}
        </Button>{' '}
        {!status.conflict && (
