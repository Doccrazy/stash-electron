...
export default withTrans<Props>('component.authHistoryPopup')(
  ({ t, open, nodePath, history, filterOptions, filterNodeId, onFilter, onClose }) => {
    return (
      <Modal size="lg" isOpen={open} toggle={onClose} returnFocusAfterClose={false}>
        ...
        <ModalBody>
          {!nodePath && (
            <UncontrolledDropdown tag="span" className={styles.filter}>
              {filterNodeId ? (
                <a
                  href="#"
                  className="text-danger"
                  title={t('common.filter.clear')}
                  onClick={(ev) => {
                    ev.preventDefault();
                    onFilter();
                  }}
              ...
              <DropdownMenu>
                {filterOptions.map((o) => (
                  <DropdownItem key={o.nodeId} active={o.nodeId === filterNodeId} onClick={() => onFilter(o.nodeId)}>
                    {o.title}
                  </DropdownItem>
                ))}
              </DropdownMenu>
            ...
            <table className="table table-sm table-sticky">
              ...
              <tbody>
                <ItemLimiter
                  items={history}
                  item={(entry) => (
                    <tr key={`${entry.hash}/${entry.nodeId}`}>
                      {!nodePath && <td>{formatPathSpaced(entry.path)}</td>}
                      <td>
                        <ul className={styles.userList}>
                          {entry.added.map((username) => (
                            <li key={username} className={styles.added}>
                              {username}
                            </li>
                          ))}
                          {entry.removed.map((username) => (
                            <li key={username} className={styles.removed}>
                              {username}
                            </li>
                          ))}
                        </ul>
