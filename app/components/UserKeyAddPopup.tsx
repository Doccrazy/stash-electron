...
export default withTrans<Props>('component.userKeyAddPopup')(
  ({ t, open, feedback, valid = true, value, privateKeyLoaded, onChange, onUsePrivateKey, onLoadKey, onConfirm, onClose }) => (
    <Modal isOpen={open} toggle={onClose} returnFocusAfterClose={false}>
        ...
        <Form id="editForm" onSubmit={onConfirm}>
          ...
          <FormGroup>
            <div className="pull-right">
              {privateKeyLoaded && (
                <Button size="sm" onClick={onUsePrivateKey}>
                  {t('.action.usePrivateKey')}
                </Button>
              )}{' '}
              <Button size="sm" onClick={onLoadKey}>
                {t('.action.loadKey')}
              </Button>
            </div>
            ...
            <Input
              ...
              onChange={(ev) => onChange({ ...value, publicKey: ev.target.value })}
              style={{ height: 160 }}
            />
          </FormGroup>
          <FormGroup>
      ...
      <ModalFooter>
        ...
        <Button type="submit" form="editForm" color="success" disabled={!valid}>
          {t('action.user.add')}
        </Button>{' '}
