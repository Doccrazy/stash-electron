...
export default withTrans<Props>('component.exportPopup')(
  ({ t, open, settings = { masterKey: '', repeatMasterKey: '' }, status, statusMessage, onChangeSettings, onExport, onClose }) => (
        ...
        <Form>
          <FormGroup>
            ...
            <FocusingInput
              ...
              type="password"
              id="masterKey"
              invalid={!settings.masterKey}
              value={settings.masterKey || ''}
              onChange={(ev) => onChangeSettings({ ...settings, masterKey: ev.target.value })}
            />
          ...
          <FormGroup>
            ...
            <Input
              type="password"
              id="masterKey"
              invalid={settings.masterKey !== settings.repeatMasterKey}
              value={settings.repeatMasterKey || ''}
              onChange={(ev) => onChangeSettings({ ...settings, repeatMasterKey: ev.target.value })}
            />
          </FormGroup>
        </Form>
      </ModalBody>
      <ModalFooter>
        <div className={status ? STATUS_COLOR[status] : ''} style={{ flexGrow: 1 }}>
          {statusMessage}
        </div>
        {status !== 'success' && (
          <Button color="primary" disabled={status === 'progress' || !valid(settings)} onClick={onExport}>
