...
export default withTrans<Props>('component.moveFolderPopup')(
  ({ t, open, sourcePath, targetPath, canMove, canMerge, onMove, onMerge, onClose }) => (
    <Modal isOpen={open} toggle={onClose} returnFocusAfterClose={false}>
      ...
      <ModalBody>
        ...
        {!canMove && <p className="text-warning">{t('.warning.cannotMove')}</p>}
        {canMerge && (
          <p>
            {canMove && (
              <span>
        ...
        {!canMerge && <p className="text-warning">{t('.warning.cannotMerge')}</p>}
      </ModalBody>
      <ModalFooter>
        {canMerge && (
          <Button autoFocus={!canMove} color="warning" onClick={onMerge}>
            <i className="fa fa-code-fork" /> {t('.action.merge')}
          </Button>
        )}{' '}
        {canMove && (
          <Button autoFocus color="primary" onClick={onMove}>
            <i className="fa fa-arrow-left" /> {t('.action.move')}
          </Button>
        )}{' '}
        <Button autoFocus={!canMove && !canMerge} color="secondary" onClick={onClose}>
