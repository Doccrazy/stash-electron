...
export default withTrans<Props>('component.userKeyTable')(({ t, keysByUser, currentUser, keyFormat, onDelete, onToggleKeyFormat, onCopyKey }) => (
  <table className={`table table-hover table-sm table-sticky ${styles.table}`}>
    <thead>
      <tr>
        <th>{t('common.column.username')}</th>
        <th className="clickable" title="Toggle key display" onClick={onToggleKeyFormat}>{t('.column.publicKey')}</th>
