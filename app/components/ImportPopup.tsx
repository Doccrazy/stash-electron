...
export default withTrans<Props>('component.importPopup')(
  ({ t, open, settings = { masterKey: '', keyFile: '' }, status, statusMessage, onChangeSettings, onImport, onClose }) => (
    <Modal isOpen={open} toggle={onClose} returnFocusAfterClose={false}>
      ...
      <ModalFooter>
        ...
        {status !== 'success' && (
          <Button color="primary" disabled={status === 'progress'} onClick={onImport}>
            {t('.action.import')}
          </Button>
        )}{' '}
